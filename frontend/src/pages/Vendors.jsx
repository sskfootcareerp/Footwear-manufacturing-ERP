...
const EMPTY_FORM = {
  ...
  address: "", payment_terms_days: 30, active: true, notes: "",
};
...
export default function Vendors() {
  ...
  const load = async () => {
    try {
      const { data } = await http.get("/vendors", { params: { include_inactive: showInactive } });
      setVendors(data || []);
    } catch (e) {
  ...
  const filtered = vendors.filter(v => {
    ...
    return `${v.name} ${v.gstin || ""} ${v.contact_person || ""}`.toLowerCase().includes(q);
  });
  ...
  const avgTerms = active.length
    ? Math.round(active.reduce((s, v) => s + (v.payment_terms_days || 30), 0) / active.length)
    : 0;

  const openAdd = () => { setForm(EMPTY_FORM); setError(""); setDrawer({ mode: "add" }); };
  const openEdit = (v) => {
    setForm({
      name: v.name || "", gstin: v.gstin || "",
      contact_person: v.contact_person || "", phone: v.phone || "",
      address: v.address || "", payment_terms_days: v.payment_terms_days ?? 30,
      active: v.active !== false, notes: v.notes || "",
    });
    setError(""); setDrawer({ mode: "edit", vendor: v });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const body = { ...form, payment_terms_days: Number(form.payment_terms_days) };
      if (drawer.mode === "add") {
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          canWrite && (
            <BtnPrimary onClick={openAdd} data-testid="add-vendor-btn">
      ...
      <div className="p-8 space-y-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <Tile label="Net 30 or Less" value={shortTerms.length} accent="#C27842" />
          <Tile label="Avg. Payment Terms" value={active.length ? `${avgTerms}d` : "—"} accent="#2563EB" />
        </div>
        ...
        <Card className="overflow-hidden" data-testid="vendors-card">
          <div className="px-5 py-3 border-b-2 border-slate-200 flex items-center justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                className="border-2 border-slate-300 px-3 py-1.5 text-sm focus:border-[#C27842] outline-none w-60"
              />
              <button
                onClick={() => setShowInactive(v => !v)}
                data-testid="toggle-inactive-btn"
                className={`text-xs uppercase tracking-wider font-bold px-3 py-1.5 border-2 transition-colors ${
                  showInactive
          ...
          {filtered.length === 0 ? (
            <div className="p-16 text-center text-slate-400 text-sm" data-testid="vendors-empty">
              ...
              <table className="w-full text-sm" data-testid="vendors-table">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr className="text-left text-[10px] uppercase tracking-wider text-slate-600">
                    ...
                    <th className="px-4 py-2 font-bold text-right">Payment Terms</th>
                  ...
                  {filtered.map(v => (
                    ...
                    >
                      <td className="px-4 py-3">
                        ...
                        {v.address && (
                          <div className="text-[11px] text-slate-400 mt-0.5 truncate max-w-[200px]">{v.address}</div>
                        )}
                      </td>
                      <td className="px-4 py-3 font-mono text-slate-600">{v.gstin || "—"}</td>
                      <td className="px-4 py-3 text-slate-700">{v.contact_person || "—"}</td>
                      <td className="px-4 py-3 font-mono text-slate-600">{v.phone || "—"}</td>
                      <td className="px-4 py-3 text-right">
                        <span className={`font-mono font-bold text-sm ${(v.payment_terms_days || 30) <= 30 ? "text-[#C27842]" : "text-slate-700"}`}>
                ...
                <tfoot className="bg-slate-900 text-white">
                  <tr>
                    <td className="px-4 py-3 font-bold uppercase text-[10px] tracking-wider text-[#C27842]">Total</td>
      ...
      {/* Add / Edit Drawer */}
      {drawer && (
        <div className="fixed inset-0 z-50 flex justify-end" data-testid="vendor-drawer">
          ...
          <div className="relative bg-white w-full max-w-lg h-full flex flex-col shadow-2xl border-l-2 border-slate-200 overflow-y-auto">
            <div className="bg-[#0F172A] text-white px-6 py-4 flex items-center justify-between shrink-0">
              <div>
                ...
                <div className="text-lg font-bold">
                  {drawer.mode === "add" ? "Add New Vendor" : `Edit: ${drawer.vendor?.name}`}
                </div>
              </div>
              <button onClick={closeDrawer} className="hover:bg-white/10 p-1" data-testid="drawer-close">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 p-6 space-y-4">
              ...
              <FormField label="Vendor Name *">
                <input data-testid="vendor-name-input" {...field("name")} placeholder="e.g. Sharma Leather Supplies"
                  className="w-full border-2 border-slate-300 px-3 py-2 text-sm focus:border-[#C27842] outline-none" />
              </FormField>

              <FormField label="GSTIN">
                <input data-testid="vendor-gstin-input" {...field("gstin")} placeholder="e.g. 27AADCB2230M1ZT"
              ...
              <div className="grid grid-cols-2 gap-3">
                <FormField label="Contact Person">
                  <input data-testid="vendor-contact-input" {...field("contact_person")} placeholder="e.g. Ramesh Sharma"
                    className="w-full border-2 border-slate-300 px-3 py-2 text-sm focus:border-[#C27842] outline-none" />
                </FormField>
                <FormField label="Phone">
                  <input data-testid="vendor-phone-input" {...field("phone")} placeholder="+91 98765 43210"
                    className="w-full border-2 border-slate-300 px-3 py-2 text-sm font-mono focus:border-[#C27842] outline-none" />
                </FormField>
              </div>

              <FormField label="Address">
                <textarea data-testid="vendor-address-input" {...field("address")} rows={3}
              ...
              <FormField label="Payment Terms (days)">
                <input data-testid="vendor-terms-input" type="number" min={0} max={365}
                  value={form.payment_terms_days}
                  onChange={(e) => setForm(f => ({ ...f, payment_terms_days: e.target.value }))}
                  className="w-full border-2 border-slate-300 px-3 py-2 text-sm font-mono focus:border-[#C27842] outline-none" />
                <div className="text-[11px] text-slate-400 mt-1">Number of credit days e.g. 30 = Net 30.</div>
              </FormField>

              <FormField label="Notes">
                <textarea data-testid="vendor-notes-input" {...field("notes")} rows={2}
              ...
              {drawer.mode === "edit" && (
                <div className="flex items-center gap-3">
                  <label className="text-[10px] uppercase tracking-wider font-bold text-slate-600">Active</label>
                  <button type="button" data-testid="vendor-active-toggle"
            ...
            <div className="shrink-0 border-t-2 border-slate-200 px-6 py-4 flex items-center justify-between bg-slate-50">
              <BtnSecondary onClick={closeDrawer} data-testid="drawer-cancel">Cancel</BtnSecondary>
              <BtnPrimary onClick={handleSave} disabled={saving} data-testid="drawer-save">
                {saving ? "Saving…" : drawer.mode === "add" ? "Create Vendor" : "Save Changes"}
              </BtnPrimary>
            </div>
          </div>
        </div>
      ...
      {/* Deactivate confirm */}
      {deactivateFor && (
        <div className="fixed inset-0 z-50 bg-black/50 grid place-items-center p-4" data-testid="deactivate-dialog">
          <div className="bg-white border-2 border-slate-200 shadow-2xl w-full max-w-sm p-6 space-y-4">
            <div className="flex items-start gap-3">
              <AlertCircle className="w-5 h-5 text-red-600 mt-0.5 shrink-0" />
              <div>
                <div className="font-bold text-slate-900">Deactivate vendor?</div>
                <div className="text-sm text-slate-500 mt-1">
                  <b>{deactivateFor.name}</b> will be marked inactive. Existing purchase history is preserved.
                  You can reactivate by editing the vendor.
                </div>
              </div>
            </div>
            <div className="flex gap-3 justify-end">
              <BtnSecondary onClick={() => setDeactivateFor(null)} data-testid="deactivate-cancel">Cancel</BtnSecondary>
              <button onClick={handleDeactivate} data-testid="deactivate-confirm"
...
function Tile({ label, value, accent }) {
  return (
    <Card className="p-5 relative overflow-hidden">
      <div className="text-[10px] uppercase tracking-[0.2em] font-bold" style={{ color: accent }}>{label}</div>
